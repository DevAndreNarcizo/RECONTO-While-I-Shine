class_name EncantoMeleeFrontal
extends EncantoBase
## MELEE_FRONTAL — golpe em área à frente do player, na direção de facing.
## (Cipó Chicoteante.) Placeholder: retângulo verde que pisca no golpe.
...
func _attack() -> void:
	...
	_shape.size = Vector2(HIT_WIDTH, HIT_HEIGHT) * area()
